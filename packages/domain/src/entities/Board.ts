...
import type { BoardId, ColumnId, TicketId } from '../common/Types';
import { ERROR_CODES } from '../errors/ErrorCodes';
import { DomainEventAggregateRoot } from '../events/DomainEventAggregateRoot';
import { TicketAddedEvent, TicketMovedEvent } from '../events/DomainEvents';
import { IBoardInternal } from './BoardSchema';
...
export class Board extends DomainEventAggregateRoot implements IBoardInternal {
    ...
    @Expose() get id(): BoardId {
        return this._id;
    }
    private set id(id: BoardId) {
        this._id = id;
    }

    @Expose() get columns(): ReadonlyArray<Column> {
        return this._columns;
    }
    private set columns(columns: Column[]) {
        this._columns = columns;
    }

    @Expose() get version(): number {
    ...
    public getTickets(specs?: {
        ...
        const matchingColumns = columnSpec
    ...
    public addTicket(ticket: Ticket): Result<void> {
        return this._getColumn(ticket.columnId).bind((column) => {
            const ticketAddSpec = new TicketCanBeAddedSpec(ticket);
            return column._addTicket(ticketAddSpec).map(() => {
                this.addDomainEvent(
                    new TicketAddedEvent({
                        boardId: this.id,
                        ticketId: ticket.id,
                    })
                );
    ...
    public moveTicket(ticketId: TicketId, targetColumnId: ColumnId): Result<void> {
        return this._getTicket(ticketId).bind((ticket) =>
            this._getSourceAndTargetColumns(ticket.columnId, targetColumnId).bind(
                ([sourceCol, targetCol]) => {
    ...
    public _addColumn(spec: ColumnCanBeAddedSpec): Result<void> {
        if (this._columns.some((col) => !spec.isSatisfiedBy(col))) {
            return Result.fail(ERROR_CODES.UIC01);
        }
        this._columns.push(spec.columnToAdd);
        return Result.ok();
    }
    ...
    private _getSourceAndTargetColumns(
        sourceId: ColumnId,
        targetId: ColumnId
