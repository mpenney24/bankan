...
import { BoardServiceFacade } from './BoardServiceFacade';

describe('BoardServiceFacade', () => {
    let boardServiceFacade: BoardServiceFacade;
    let mockBoardRepository: IFirestoreRepository<Board>;
    ...
    beforeEach(() => {
        ...
        mockBoardRepository = h.mock<IFirestoreRepository<Board>>({
        ...
        moveTicketHandler = new MoveTicketHandler(
            mockBoardRepository,
            mockEventDispatcher
        );
    ...
    describe('#moveTicket', () => {
        it('should report a ticket successfully moved column with an event dispatch and db persist', async () => {
            const command: MoveTicketCommand = {
                boardId: board.id,
                ticketId: ticketBacklog.id,
                targetColumnId: h.COLUMN_ID_IN_PROGRESS,
            };
        ...
        it('should report a ticket failed to move to invalid target column with no event dispatch or db persist', async () => {
            const command: MoveTicketCommand = {
                boardId: board.id,
                ticketId: ticketBacklog.id,
                targetColumnId: h.COLUMN_ID_INVALID,
            };
        ...
        it('should fail to retrieve the board', async () => {
            ...
            mockBoardRepository.getById = vi
                .fn()
                .mockResolvedValue(Result.fail(ERROR_CODES.F01(command.targetColumnId)));
    ...
    describe('#addTicket', () => {
        it('should report a ticket successfully added to column with an event dispatch and db persist', async () => {
            ...
            const createTicketPayload: ICreateTicket = h.createTicketPayload(
                h.COLUMN_ID_BACKLOG
            );

            const command: AddTicketCommand = {
                boardId: board.id,
                createTicketPayload,
            };
            ...
            const savedNewTicket = columnBacklog.tickets.find(
                (tick) => tick.name === createTicketPayload.name
            )!;
        ...
        it('should report a ticket failed to add to invalid column with no event dispatch or db persist', async () => {
            const ticketCount = columnBacklog.tickets.length;
        ...
        it('should fail to retrieve the board', async () => {
            ...
            mockBoardRepository.getById = vi
                .fn()
                .mockResolvedValue(
                    Result.fail(ERROR_CODES.F01(createTicketPayload.columnId))
                );
